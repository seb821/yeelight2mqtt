...
def on_message(client, userdata, message):
    ...
    publish.single("yeelight2mqtt/properties/{0}".format(ip), payload=payload, hostname=broker, port=port, auth=auth)

subscribe.callback(on_message, "yeelight2mqtt/bulb/#", hostname=broker, port=port, auth=auth)
